...
class ControllerCommonSeoPro extends Controller {
		...
		if (!isset($this->request->get['_route_'])) {
			...
			if (count($rows) == sizeof($parts)) {
				...
				foreach ($parts as $part) {
					$surl = explode('&', $queries[$part], 2);
					if (count($surl) > 1) {
						$url1=explode('=', $surl[0], 2);
						$this->request->get[$url1[0]] = $url1[1];
						$url2=explode('=', $surl[1], 2);
						$this->request->get[$url2[0]] = $url2[1];
			...
			} elseif ((isset($this->request->get['manufacturer_id'])) || (isset($this->request->get['catId']))) {
	...
	public function rewrite($link) {
		...
		if (count($queries) == 2) {
			...
			$q1 = explode('=', $queries[0], 2);
			$q2 = explode('=', $queries[1], 2);
			if (($q1[0]=='catId') && ($q2[0]=='manufacturer_id')) {
				$queries[0] ='manufacturer_id='.$q2[1].'&catId='.$q1[1];
				unset($queries[1]);
			} elseif (($q2[0]=='catId') && ($q1[0]=='manufacturer_id')) {
				$queries[0] ='manufacturer_id='.$q1[1].'&catId='.$q2[1];
				unset($queries[1]);
			}
