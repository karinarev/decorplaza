	...
	private function generateYml() {
		if ($this->config->get('yandex_yml_status')) {
			...
			foreach ($categories as $category) {
				$this->setCategory($category['name'], $category['category_id'], (int)$category['parent_id']);
			}
			...
			$products = $this->model_export_yandex_yml->getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers,'market');
...
//				$data['barcode'] = $product['sku'];
			...
			$allowed_categories_mas = explode(",",$allowed_categories);
			//var_dump($allowed_categories_mas);exit;
			foreach ($this->categories as $keycategorie => $categorie) {
				if (in_array($categorie['id'],$allowed_categories_mas)) {
					$this->categories[$keycategorie]['export'] = 1;
				} else {
					unset($this->categories[$keycategorie]);
				}
			}
			//$this->categories = array_filter($this$this->categories, array($this, "filterCategory"));
