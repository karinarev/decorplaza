...
class ControllerFeedYandexYml3 extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('yandex_yml_status')) {
			...
			$products = $this->model_export_yandex_yml->getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers,'partner');
...
//				$data['barcode'] = $product['sku'];
		...
		foreach ($offers_array as $i=>$data) {
			if (count($soptions)) {
				foreach ($soptions as $option) {
					...
					$data_arr['param'][$size_option_name] = explode($data_arr['param'][$size_option_name],",");
	...
	private function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'market_category'=>0, 'picture'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0, 'size'=>1);
