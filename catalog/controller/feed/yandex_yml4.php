...
//++++ Config section ++++
//Версия модуля
define ('YANDEX_YML_VERSION', '1.6.3');
...
class ControllerFeedYandexYml4 extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('yandex_yml4_status')) {
			...
			$this->load->model('export/yandex_yml4');
			$this->load->model('localisation/currency');
			$this->load->model('tool/image');

			// Магазин
			$this->setShop('name', $this->config->get('config_name'));
			//$this->setShop('name', $this->config->get('yandex_yml4_shopname'));
			$this->setShop('company', $this->config->get('config_owner'));
			//$this->setShop('company', $this->config->get('yandex_yml4_company'));
			...
			$offers_currency = $this->config->get('yandex_yml4_currency');
			...
			$datamodel = $this->config->get('yandex_yml4_datamodel');
			
			// Категории
			$categories = $this->model_export_yandex_yml4->getCategory();
			...
			$in_stock_id = $this->config->get('yandex_yml4_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('yandex_yml4_out_of_stock'); // id статуса товара "Нет на складе"
			$vendor_required = ($datamodel == 'vendor_model'); // true - только товары у которых задан производитель, необходимо для 'vendor.model' 

			$pickup = ($this->config->get('yandex_yml4_pickup') ? 'true' : false);
			
			if ($this->config->get('yandex_yml4_delivery_cost') != '') {
				$local_delivery_cost = intval($this->config->get('yandex_yml4_delivery_cost'));
			...
			$store = ($this->config->get('yandex_yml4_store') ? 'true' : false);
			$unavailable = $this->config->get('yandex_yml4_unavailable');

			$allowed_categories = $this->config->get('yandex_yml4_categories');
			
			$allowed_manufacturers = $this->config->get('yandex_yml4_manufacturers');
			$blacklist = $this->config->get('yandex_yml4_blacklist');
			$products = $this->model_export_yandex_yml4->getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers,'partner');

			$numpictures = $this->config->get('yandex_yml4_numpictures');
			if ($numpictures > 1) {
				//++++ Дополнительные изображения товара ++++
				$product_images = $this->model_export_yandex_yml4->getProductImages($numpictures - 1);
				//---- Дополнительные изображения товара ----
			}
			
			$all_attributes = $this->model_export_yandex_yml4->getAttributes($this->config->get('yandex_yml4_attributes'));
			$this->optioned_name = $this->config->get('yandex_yml4_optioned_name');
			
			$yandex_yml_categ_mapping = unserialize($this->config->get('yandex_yml4_categ_mapping'));
			
			$this->color_options = explode(',', $this->config->get('yandex_yml4_color_options'));
			$this->size_options = explode(',', $this->config->get('yandex_yml4_size_options'));
			$this->size_units = $this->config->get('yandex_yml_size_units') ? unserialize($this->config->get('yandex_yml4_size_units')) : array();
...
//				$data['cbid'] = 15;
				...
				$data['group_id'] = $product['model'];
				$product_description = strip_tags($product[YANDEX_YML_DESCRIPTION_FIELD]);
				if (YANDEX_YML_SHORTER_DESCRIPTION > 0) {
					$product_description = mb_substr($product_description, 0, YANDEX_YML_SHORTER_DESCRIPTION);
				}
				$data['description'] = $product_description;
				//data['description'] = "";
				$sales_notes = $this->config->get('yandex_yml4_sales_notes');
				if ($sales_notes) {
...
//				$data['barcode'] = $product['sku'];
				if ($numpictures > 0) {
					if ($product['image']) {
						...
						//$data['picture'] = array($product['image']);
					}
					//++++ Дополнительные изображения товара ++++
				...
				*/
				...
				$attributes = $this->model_export_yandex_yml4->getProductAttributes($product['product_id']);
				if (count($attributes) > 0) {
					foreach ($attributes as $attr) {
						if ($attr['attribute_id'] == $this->config->get('yandex_yml4_adult')) {
							$data['adult'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('yandex_yml4_manufacturer_warranty')) {
							$data['manufacturer_warranty'] = 'true';
						}
						elseif ($attr['attribute_id'] == $this->config->get('yandex_yml4_country_of_origin')) {
				...
				//---- Атрибуты товара ----
				/*$soptions = $this->model_export_yandex_yml4->getProductOptions($this->size_options, $product['product_id']);
				$size = array();
				if (count($soptions) > 0) {
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		$offers_array = array();
		/*
		$coptions = $this->model_export_yandex_yml4->getProductOptions($this->color_options, $product['product_id']);
		if (!count($coptions))
			return false;
			...
			if (count($soptions)>0) {
				...
				foreach ($soptions as $option) {
					...
					elseif ($option['price_prefix'] == '-')
						$data_arr['price']-= $option['price'];
					$offers_array[] = $data_arr;
					$data_arr['quantity'] = $option['quantity'];

					$data_arr['id'] = $data['group_id']."-".str_pad($idx, 1, '0', STR_PAD_LEFT);
				...
				$data['id'] = $data['group_id']."-".str_pad($idx, 1, '0', STR_PAD_LEFT);
				$data['price'] = number_format($this->currency->convert($this->tax->calculate($data['price'], $product['tax_class_id'], $this->config->get('config_tax')), $shop_currency, $offers_currency), $decimal_place, '.', '');
				if ($data['price'] > 0) {
					$this->setOffer($data);
				}
	...
	private function setOffer($data) {
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0,'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'downloadable'=>0, 'adult'=>0, 'barcode'=>0, 'quantity'=>1));
...
//		}
		...
		foreach ($allowed_tags as $key => $value) {
			...
			//$offer['data']['weight'] = (int)$data['weight'];
		}
	...
	private function array2Tag($tags) {
		...
		foreach ($tags as $key => $value) {
			if (is_array($value)) {
				...
				//$retval .= '<' . $key . '>' . var_dump($tags) . '</' . $key . '>' . $this->eol;
			}
		}
