<?php
class ModelExportYandexYml4 extends Model {
	...
	public function getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required = true, $allowed_manufacturers = '',$type) {
		if ($type == 'partner') {
			$sqltype = " AND p.exportpartners = '1' ";
		} else if ($type == 'market') {
			$sqltype = " AND p.exportmarket = '1' ";
		}
		
		$query = $this->db->query("SELECT
			p.*, pd.name, pd.description, pd.meta_description, m.name AS manufacturer, p2c.category_id, IFNULL(ps.price, p.price) AS price
	...
	public function getProductOptions($option_ids, $product_id) {
		...
		/*$qw = "SELECT pov.option_id, od.name AS option_name, ovd.name, pov.quantity, pov.price, pov.price_prefix
		...
		$query = $this->db->query("SELECT pov.option_id, od.name AS option_name, ovd.name, pov.quantity, pov.price, pov.price_prefix
			...
			LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (pov.option_value_id = ovd.option_value_id)
			LEFT JOIN " . DB_PREFIX . "option_description od ON (od.option_id = pov.option_id) AND (od.language_id = '$lang')
			WHERE pov.option_id IN (". implode(',', $ids) .") AND pov.quantity>0 AND pov.product_id = '". (int)$product_id."'
				AND ovd.language_id = '$lang'");
				/*
		$query = $this->db->query("SELECT pov.option_id, od.name AS option_name, ovd.name, pov.quantity, pov.price, pov.price_prefix
			FROM " . DB_PREFIX . "product_option_value pov 
