...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, p.newItem, p.bestSaller, p.bestSeller, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < '" . $this->NOW . "') AND (pd2.date_end = '0000-00-00' OR pd2.date_end > '" . $this->NOW . "')) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		...
		if ($query->num_rows) {
			return array(
				...
				'name'             => $query->row['name'],
				'functions'        => $query->row['functions'],
				'description'      => $query->row['description'],
				'video'            => $query->row['video'],
				'video_description'=> $query->row['video_description'],
				...
				'newItem'           => $query->row['newItem'],
				'bestSaller'           => $query->row['bestSaller'],
	...
	public function checkActions() {
		...
		foreach ($products as $product) {
			...
			$totalactions = $this->getTotalActions($product['product_id']);
			
		
			if ($totalactions == 0) {
				$this->delRaspr($product['product_id']);
			} else {
				$this->addRaspr($product['product_id']);
			}
	...
	public function getTotalActions($product_id) {
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special WHERE ((product_id = '" . (int)$product_id . "') AND (date_start = '0000-00-00' OR date_start < '" . $this->NOW . "') AND (date_end = '0000-00-00' OR date_end > '" . $this->NOW . "'))");
		
		return count($query->rows);
	
	}
	
	public function delRaspr($product_id) {
		//echo("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id='84'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id='84'");
		//exit;
	}
	
	public function addRaspr($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id='84'");
		$raspr = count($query-rows);
		if ($raspr == 0) {
...
//		$product_data['sql']=$sql;
	...
	public function getBestSellerProducts1($limit) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		if (!isset($product_data)) { 
			...
			$dateNowFix = date("Y-m-d");
			$query = $this->db->query("SELECT product_id FROM oc_productday WHERE date = '$dateNowFix' LIMIT 1");
			$arProducts = array(); // Создаем массив
			
			foreach ($query->rows as $result) { 		
				if (count($result['product_id']) > 0) {
			...
			if (empty($productID)){
					...
					shuffle($arProducts);
					$productID = $arProducts[0];
					
					$query = $this->db->query("INSERT INTO `" . DB_PREFIX . "productday` (`id`, `date`, `product_id`) VALUES ('', '" . $this->NOW . "', '".$productID."')");
			}		
	...
	public function getBestSellerProducts($limit) {
		...
		if (!isset($product_data)) { 
			...
			/*$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.bestSaller = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);*/
			
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p WHERE p.status = '1' AND p.bestSaller = '1' AND p.date_available <= '" . $this->NOW . "' ORDER BY p.product_id DESC LIMIT " . (int)$limit);
	...
	public function getCityForms($city='Москва') {
