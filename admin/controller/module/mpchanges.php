...
class ControllerModuleMpchanges extends Controller {
    ...
    private function loadFilter(){
        ...
        $this->mpfilter["manufacturer_weight"] = 0;
        if (isset($this->request->post['manufacturer_weight'])) {
            $this->mpfilter["manufacturer_weight"] = $this->request->post['manufacturer_weight'];
        }
        $this->mpfilter["manufacturer_status"] = -1;
        if (isset($this->request->post['manufacturer_status'])) {
            $this->mpfilter["manufacturer_status"] = $this->request->post['manufacturer_status'];
        }
        $this->mpfilter["manufacturer_new_name"] = "";
        if (isset($this->request->post['manufacturer_new_name'])) {
            $this->mpfilter["manufacturer_new_name"] = $this->request->post['manufacturer_new_name'];
        }
        $this->mpfilter["manufacturer_old_name"] = "";
        if (isset($this->request->post['manufacturer_old_name'])) {
            $this->mpfilter["manufacturer_old_name"] = $this->request->post['manufacturer_old_name'];
        }
        ...
        $this->mpfilter["weight_diff"] = '-';
        if (isset($this->request->post['weight_diff'])) {
            switch ($this->request->post['weight_diff']){
                case '-':
                    $this->mpfilter["weight_diff"] = '-';
                    break;
                case '+':
    ...
    $this->load->model('catalog/manufacturer');
        ...
        $this->data['button_change_weight'] = $this->language->get('button_change_weight');
        $this->data['button_change_status'] = $this->language->get('button_change_status');
        $this->data['button_change_name'] = $this->language->get('button_change_name');
        ...
        $this->data['entry_old_name'] = $this->language->get('entry_old_name');
        $this->data['entry_new_name'] = $this->language->get('entry_new_name');
        ...
        $this->data['action_change_weight'] = $this->url->link('module/mpchanges/changeweight', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_change_status'] = $this->url->link('module/mpchanges/changestatus', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_change_name'] = $this->url->link('module/mpchanges/changename', 'token=' . $this->session->data['token'], 'SSL');
    ...
    public function changename() {
        ...
        if ($this->mpfilter["products"]){
            foreach ($this->mpfilter["products"] as $product){
                ...
                if (in_array($product['product_id'],$this->mpfilter['change_ids']) or $this->mpfilter['change_all']){
                    ...
                    if ($this->mpfilter["manufacturer_new_name"] != ""){
                        $store_new_name = $this->mpfilter["manufacturer_new_name"];
                        if ($this->mpfilter["manufacturer_old_name"] != "") {

                            $store_old_name = $this->mpfilter["manufacturer_old_name"];
                           $store_new_name = str_replace ($this->mpfilter["manufacturer_old_name"],$this->mpfilter["manufacturer_new_name"],$product['name']);

                        }

                        

                        $this->model_module_mpchanges->editProductName($product['product_id'], $store_new_name);
                        $changed[$product['product_id']]['name'][$this->mpfilter["store_id"]] = $store_new_name;
                    }
        ...
        $this->mp_message['message'][] = $this->language->get('success_name_changed');
    ...
    public function changestatus() {
        ...
        if ($this->mpfilter["products"]){
            foreach ($this->mpfilter["products"] as $product){
                ...
                if (in_array($product['product_id'],$this->mpfilter['change_ids']) or $this->mpfilter['change_all']){
                    ...
                    if ($this->mpfilter["manufacturer_status"] >= 0){
                        $store_status = $this->mpfilter["manufacturer_status"];

                        

                        $this->model_module_mpchanges->editProductStatus($product['product_id'], $store_status);
                        $changed[$product['product_id']]['status'][$this->mpfilter["store_id"]] = $store_status;
                    }
        ...
        $this->mp_message['message'][] = $this->language->get('success_status_changed');
        $this->mp_message['message'] = implode('<br/> ', $this->mp_message['message']);
    ...
    public function changeweight() {
        ...
        if ($this->mpfilter["products"]){
            foreach ($this->mpfilter["products"] as $product){
                ...
                if (in_array($product['product_id'],$this->mpfilter['change_ids']) or $this->mpfilter['change_all']){
                    ...
                    if ($this->mpfilter["manufacturer_weight"] > 0){
                        $store_weight = $this->getDiffValue($product['weight'], $this->mpfilter["weight_diff"], $this->mpfilter["manufacturer_weight"], $this->mpfilter["change_type"]);

                        if ($store_weight < 0) $store_weight = 0;

                        $this->model_module_mpchanges->editProductWeight($product['product_id'], $store_weight);
                        $changed[$product['product_id']]['weight'][$this->mpfilter["store_id"]] = $store_weight;
                    }
        ...
        $this->mp_message['message'][] = $this->language->get('success_weight_changed');
        $this->mp_message['message'] = implode('<br/> ', $this->mp_message['message']);
