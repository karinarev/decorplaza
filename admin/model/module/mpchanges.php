...
class ModelModuleMpchanges extends Model {
    ...
    public function editProductWeight($product_id, $weight) {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET weight = '" . (float)$weight . "' WHERE product_id = " . (int)$product_id);
    }
    ...
    public function editProductName($product_id, $name) {
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = '" . $this->db->escape($name) . "' WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' AND product_id = " . (int)$product_id);
    }

    public function editProductStorePrice($product_id, $store_id, $price) {
    ...
    public function getProducts($data = array()) {
        $sql = "SELECT pd.name, p.product_id, p.price, p.quantity, p.weight, !isnull(pd2.price) as discount, !isnull(ps.price) as special FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
