<?php
class ModelModuleParseimport2 extends Model {
	...
	public function insertProduct($product) {
		...
		if (strlen($product[5]) > 0) {
			...
			if (strlen($product[6]) > 0) {
				$priceoptions = explode(",",$product[6]);
			}
			$option_id = 13;
			...
			foreach ($options as $key => $option) {
				if ($this->getOptionValue($option) > 0) {
					...
					if (strlen($product[6]) > 0) {
						$tekpriceoption = $priceoptions[$key];
						if ($tekpriceoption > $product[4]) {
							$inspriceoption = $tekpriceoption-$product[4];
						} else {
							$inspriceoption = 0;
						}
					} else {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', option_value_id = '" . (int)$option_value_id . "', quantity = '1', subtract = '0', price = '" . $inspriceoption . "', price_prefix = '+', points = '0', points_prefix = '+', weight = '0.00000000', weight_prefix = '+'");
				}	
			}
		}
