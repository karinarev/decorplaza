...
class ModelSaleCoupon extends Model {
	public function addCoupon($data) {
		if (isset($data['code1'])) {
			...
			$code1 = (int)$this->db->escape($data['code1']);
			$code2 = (int)$this->db->escape($data['code2']);
			$kol = $code2 - $code1 +1;
			if ($kol>0) {
				$kolzn = utf8_strlen($this->db->escape($data['code1']));
				$codetek = $code1;
				for ($i=0;$i<$kol;$i++) {
					$codetekformat = sprintf("%0".$kolzn."d",$codetek);
					$this->db->query("INSERT INTO " . DB_PREFIX . "coupon SET name = '" . $this->db->escape($data['name']) . "', code = '" . $codetekformat . "', discount = '" . (float)$data['discount'] . "', type = '" . $this->db->escape($data['type']) . "', total = '" . (float)$data['total'] . "', logged = '" . (int)$data['logged'] . "', shipping = '" . (int)$data['shipping'] . "', date_start = '" . $this->db->escape($data['date_start']) . "', date_end = '" . $this->db->escape($data['date_end']) . "', uses_total = '" . (int)$data['uses_total'] . "', uses_customer = '" . (int)$data['uses_customer'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
					$codetek = $codetek + 1;
